...
function OnPlaceTankRequest() {
	...
	if (GetMyTeam() == TEAM_1) {
		...
		PlaceTank(TANK_HEAVY, 3, 1);
		PlaceTank(TANK_HEAVY, 3, 20);
...
function GetTankDirection (TANK) {
  ReturnDirections = [];
  if (GetTileAt(TANK.m_x-1, TANK.m_y) == BLOCK_GROUND) ReturnDirections.push(DIRECTION_LEFT);
  if (GetTileAt(TANK.m_x, TANK.m_y-1) == BLOCK_GROUND) ReturnDirections.push(DIRECTION_UP);
  if (GetTileAt(TANK.m_x+1, TANK.m_y) == BLOCK_GROUND) ReturnDirections.push(DIRECTION_RIGHT);
  if (GetTileAt(TANK.m_x, TANK.m_y+1) == BLOCK_GROUND) ReturnDirections.push(DIRECTION_DOWN);
	...
	if (Source_x - 1 < Destine_x && Destine_x < Source_x + 1){
		if (Source_y < Destine_y){
			for(var y = Source_y; y < Destine_y; y++){
				if ((GetTileAt(Source_x, y) != BLOCK_GROUND) || (GetTileAt(Source_x, y) != BLOCK_WATER)) return "Up_Down";
			}
		}
		else {
	...
	if (Source_y - 1 < Destine_y && Destine_y < Source_y + 1){
		if (Source_x < Destine_x){
			for(var x = Source_x; x < Destine_x; x++){
				if ((GetTileAt(x, Source_y) != BLOCK_GROUND) || (GetTileAt(x, Source_y) != BLOCK_WATER)) return "Left_Right";
			}
		}
		else {
    ...
    direction = TankMarch();
		...
		if (TagetTanks(tempTank).length != 0)
		{
			...
			for(var j=0; j < TagetTanks(tempTank).length; j++)
			{
				...
				if (ClearShot(tempTank.m_x, tempTank.m_y, enemytank.m_x, enemytank.m_y) == null){
					continue;
				 }
				else if (ClearShot(tempTank.m_x, tempTank.m_y, enemytank.m_x, enemytank.m_y) == "Up_Down") {
					...
					if (tempTank.m_coolDown > 0) {
						direction = Opposite(Navigate(tempTank.m_x, tempTank.m_y, enemytank.m_x, enemytank.m_y)["Left_Right"]);
						shot = false;
						break;
					}
				}
				else if (ClearShot(tempTank.m_x, tempTank.m_y, enemytank.m_x, enemytank.m_y) == "Left_Right") {
					if (tempTank.m_coolDown ==0){
					...
					if (tempTank.m_coolDown >0){
						direction = Opposite(Navigate(tempTank.m_x, tempTank.m_y, enemytank.m_x, enemytank.m_y)["Up_Down"]);
						shot = true;
						break;
					}
				}
			...
			direction = GetTankDirection(tempTank)[Math.floor((Math.random() * 10))];
			shot = false;
		}
